...
//                    bg.console.log("The comment seems to be changed");
    ...
    navigate: function(url, context, fragment, keep_opened)
    {
        try {
            ...
            var url = wot.contextedurl(url, context) + fragment;

            this.execute_navigation(url, keep_opened);
        } catch (e) {
            console.error("ratingwindow.navigate: failed with ", e);
        }
    },

    execute_navigation: function(url, keep_opened) 
    {
        chrome.tabs.create({ url, active:!keep_opened },
            function(tab) {
    ...
    simple_navigate: function(url) {
        this.execute_navigation(url, false);
    },
    ...
    updatecontents: function()
    {
        ...
        // override any footer message with the one below
        _this.override_footer_user_message();
...
//	    _this.wg.update_wg_tags();
    ...
    // override any footer message with the one below
    override_footer_user_message: function() {
        var ALWAYS_SHOW = false;

        var $_wot_message = $("#wot-message"),
            $_wot_message_text = $("#wot-message-text"),
            $_user_communication = $("#user-communication");

        if (wot.OVERRIDE_FOOTER_USER_MESSAGE) {
            $_wot_message.show();
            if (ALWAYS_SHOW) { $_user_communication.show(); }
            $_wot_message.attr("msg_id", "app-andr-override");
            $_wot_message.attr("status", "override");

            $_wot_message_text.html("WOT is now available for Android!")
            $_wot_message_text.attr("url", "https://play.google.com/store/apps/details?id=com.my.wot&referrer=utm_source%3DExtension%26utm_campaign%3DFooter");
            $_wot_message_text.attr("status", "override");
            $_wot_message_text.css({
                "font-size": "14px",
                "font-weight": "500",
                "color": "#444444",
                "position": "absolute",
                "left": "77px",
                // "text-shadow": "0px 0px 1px rgba(0,0,0,0.2),0px 2px 13px rgba(0,0,0,0.1), 0px 18px 23px rgba(0,0,0,0.1)"
            });
            if ($("#wot-message-text-btn").length === 0) {
                $_wot_message.append('<div id="wot-message-text-btn"></div>');
            }
            $_wot_message_text_btn = $("#wot-message-text-btn");
            $_wot_message_text_btn.css({
                "width": "90px",
                "height": "30px",
                "background-image": 'url("skin/app_screen/app_btn_small.png")',
                "position": "absolute",
                "top": "9px",
                "right": "117px",
                "cursor": "pointer"
...
//		var _rw = wot.ratingwindow;
	...
	showhide_share_wot_screen: function(show, blurOnly) {

		var $wot_elements = $('#wot-elements');
		var $share_screen = $('#wot-share');

		if (!blurOnly) { $share_screen.toggle(show); }
		$wot_elements.toggleClass('blur', show);
	},

    showhide_wot_app_screen: function(show, blurOnly) {
        var $wot_elements = $('#wot-elements');
        var $app_screen = $('#wot-app');

        if (!blurOnly) { $app_screen.toggle(show); };
        $wot_elements.toggleClass('blur', show);
    },
...
//            $("#wot-rating-" + n + "-boundright").text(wot.i18n("testimony", item.name + "_levels_" + wot.getlevel(wot.reputationlevels, 100).name));
	...
	engage_with_share_screen: function () {
		...
		var open_points = [5, 15]; // engage with user when RW is opened these number of times
		...
		if (open_points.indexOf(opened) < 0) {
    ...
    show_app_screen: function() {
        var _rw = wot.ratingwindow;
        var app_screen = _rw.prefs.get("rw_app_screen") || { rw_opened_counter: 0, page_visited: false };
        app_screen.rw_opened_counter++;
        var open_points = [6, 16];

        _rw.prefs.set("rw_app_screen", app_screen);

        if (open_points.indexOf(app_screen.rw_opened_counter) < 0 || app_screen.page_visited) {
        ...
        // show screen here
        var bg = chrome.extension.getBackgroundPage();
        bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_APP_WIN_SHOWN, "");
        _rw.showhide_wot_app_screen(true);
    },

    show_rate_us_bar: function() {
        var SHOW_TIMER_DELAY = 2000;
    
        // supported only on Chrome
        if (wot.platform !== wot.PLATFORMS.CHROME) { return; }

        var bg = chrome.extension.getBackgroundPage();
        var _rw = wot.ratingwindow;
        var rate_us_bar = _rw.prefs.get("rw_rate_us_bar") || { rw_rate_us_bar_opened_counter: 0, bar_engaged: false };
        var open_point = 6;

        rate_us_bar.rw_rate_us_bar_opened_counter++;
        _rw.prefs.set("rw_rate_us_bar", rate_us_bar);

        var counter = rate_us_bar.rw_rate_us_bar_opened_counter;
        if (counter >= open_point && !rate_us_bar.bar_engaged) {
            if (counter === open_point) {
                $("#rate-us").css("display", "block");

                // will invoke the animation (plus will send a GA event)
                setTimeout(() => {
                    $("#rate-us").removeClass("rate-us-hidden");
                    // fire event
                    bg.wot.ga.fire_event(
                        wot.ga.categories.RW, 
                        wot.ga.actions.RW_RATE_US_BAR_FIRST_TIME_SHOWN, 
                        ""
                    );
                }, SHOW_TIMER_DELAY);
    ...
    onload: function()
    {
        ...
        // Check if dynamic protect is enabled
        var dynamic_protect = bg.wot.prefs.get("dynamic-protect-on");
        if(!dynamic_protect) {
            bg.wot.open_scorecard();
            bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_OFF_SCORE_CARD, "website");
            close();
        }

        bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_SHOWN);

        //bg.wot.print("RW_Shown");
        ...
        $("#wot-message").bind("click", function() {
            ...
            if (url) {
                ...
                if ($("#wot-message").attr("status") === "override") {  // android app footer message clicked
                    bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_APP_FOOTER_CLICKED, label);
                    wot.ratingwindow.simple_navigate(url);
                } else {
                    // all other messages
        ...
        $("#floating-message").bind("click", function() {
            var url = $("#floating-message").attr("url");
            if (url) {
                var label = wot.i18n("locale") + "__" + $(this).attr("msg_id");
                bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_MSG_CLICKED, label);
                wot.ratingwindow.navigate(url, wurls.contexts.rwmsg);
        ...
        $("#comment-register-link").bind("click", function() {
            var bg = chrome.extension.getBackgroundPage();
            bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_CLICK_REGISTER, "");
	    ...
	    $("#wot-share-close").bind("click", function() {
            ...
            $("#wot-share").bind("transitionend", function() {
                $(this).toggle(false);
            });
            $("#wot-share").attr("class", "fadeout");
		    _rw.showhide_share_wot_screen(false, true);
	    });

        // app screen
        $("#wot-app-btn").bind("click", function() {
            var app_screen = _rw.prefs.get("rw_app_screen");
            app_screen.page_visited = true;
            _rw.prefs.set("rw_app_screen", app_screen);
            bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_APP_WIN_CLICKED, "");
            wot.ratingwindow.navigate('https://play.google.com/store/apps/details?id=com.my.wot&referrer=utm_source%3DExtension%26utm_medium%3DBanner');
        });

        $("#wot-app-close-btn").bind("click", function() {
            $("#wot-app").bind("transitionend", function() {
                $(this).toggle(false);
            });
            $("#wot-app").attr("class", "fadeout");
            bg.wot.ga.fire_event(wot.ga.categories.RW, wot.ga.actions.RW_APP_WIN_DISMISS, "");
            _rw.showhide_wot_app_screen(false, true);
        });

        // "Rate-Us" bar
        var RATE_US_LINK = "https://chrome.google.com/webstore/detail/wot-web-of-trust-website/bhmmomiinigofkjcapegjjndpbikblnp/reviews";

        $("#rate-us-close-button").on("click", function() {
            var _rw = wot.ratingwindow;
            var rate_us_bar = _rw.prefs.get("rw_rate_us_bar");

            $("#rate-us").attr("class", "fade-out-ready rate-us-hidden");
            rate_us_bar.bar_engaged = true;
            _rw.prefs.set("rw_rate_us_bar", rate_us_bar);

            // fire event
            bg.wot.ga.fire_event(
                wot.ga.categories.RW, 
                wot.ga.actions.RW_RATE_US_BAR_CLOSE_BUTTON_CLICKED, 
                ""
            );
        });
        ...
        $("#rate-us-button").on("click", function() {
            ...
            bg.wot.ga.fire_event(
                wot.ga.categories.RW, 
                wot.ga.actions.RW_RATE_US_BAR_BUTTON_CLICKED, 
                ""
            );
        });
	    ...
	    // _rw.engage_with_share_screen();     // currently disabled
        // _rw.show_app_screen();              // currently disabled
        _rw.show_rate_us_bar();                // Chrome only
...
//	    _rw.wg.update_popular_tags();
    ...
    on_delete_button: function () {
        ...
        $("#change-ratings").hide();    // when a rating gets deleted, hide the "Edit Rating" button
    },
