...
var preedithost = "", preediturl = "";
var isD = false;
var intertldreg = new RegExp("xn--", "i");
var unicode = new RegExp("[\\u00a1-\\uffff]", "i");
...
frigateOverlay = {listKey:false, keyPressThis:false, keyPress:function(e) {
  if (e.key == "esc") {
    if (e) {
      e.stop();
    }
  ...
  this.hide();
  var overlay = new Element("div", {"id":"frigate-overlay", events:{click:this.cancelEdit.bind(this)}});
...
function okErr(txt, end) {
  var SM = new SimpleModal({"btn_ok":l("messAlertClose"), "onClose":end});
  SM.show({"title":l("messAlertError"), "contents":txt});
}
    ...
    if (url == -2 && j.s == -1 || url == -1 && j.s == -1 && j.h2.search(/(4|5)\d\d/g) != -1 || !(url < 0) && j.s == -1) {
      if (siteList.on && (!key || siteList.d[key].on)) {
        var sendHost = function(response) {
          chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"url", "host":site, "url":url, "list":siteList.n, "lid":params.id, "new":preedithost == ""}}, function(response) {
            if (key) {
              siteList.d[key].u = url;
            } else {
              siteList.d.push({h:site, u:url, on:true});
        ...
        if (preedithost) {
          chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"delurl", "host":preedithost, "url":preediturl, "notApply":true}}, sendHost);
        } else {
          sendHost.apply();
        }
      } else {
...
function intertld(site) {
  return intertldreg.test(site);
}
function frigateSaveSite(site, url, t, key, endfu, endfu2) {
  ...
  var asterisk = "", end = "/", sheme = "http://", sheme2 = sheme, dopPostParam = "";
  ...
  site = urlSplit[0];
  if (intertld(site)) {
    site = punycode.ToUnicode(site);
  }
  if (checkUrl(sheme + site + end)) {
    var alredyIs = false;
    var list = siteList.d;
    if (unicode.test(site)) {
    ...
    if (!alredyIs) {
      ...
      if (urlCh !== false) {
        chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"churl", "host":site, "new":preedithost == ""}}, function(response) {
          if (response && response != "friGate" && response != params.id) {
            ...
            if (t == 3) {
              frigateSaveSiteStep2({s:-1}, asterisk + site, url, key, endfu, endfu2);
            } else {
...
function frigateShowIteamList(key) {
  ...
  td[2].empty().grab(new Element("a", {"text":cutUrl(siteList.d[key].u), "class":"nolink", "rel":key, "href":"", events:{"click":noact, "dblclick":frigateEditList}}));
...
function frigateDelList(event) {
  ...
  var SM = new SimpleModal({"btn_ok":l("messListsConfirmOK"), "btn_cancel":l("messListsConfirmCancel")});
  SM.show({"model":"confirm", "callback":function() {
...
function frigateOnOffList(event) {
  ...
  if (siteList.on) {
    if (typeof siteList.d[this.rel].on == "undefined" || !siteList.d[this.rel].on) {
      chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"url", "host":siteList.d[this.rel].h, "list":siteList.n, "lid":params.id, "url":siteList.d[this.rel].u}}, onoff.bind(this));
    } else {
      chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"delurl", "host":siteList.d[this.rel].h, "url":siteList.d[this.rel].u}}, onoff.bind(this));
    }
  } else {
    onoff.apply(this);
  }
...
function frigateEditList(event) {
  ...
  if (siteList.d[this.rel].u != -5) {
    ...
    var select = new Element("select", {"id":"typecheck" + this.rel, "class":"select3"});
    select.grab(new Element("option", {"text":l("messTypeCh2"), "value":2, "selected":getOpt(siteList.d[this.rel], 2)}));
    select.grab(new Element("option", {"text":l("messTypeCh3"), "value":3, "selected":getOpt(siteList.d[this.rel], 3)}));
    var inpUrl = new Element("input", {"id":"sitehost" + this.rel, "class":"inpedit3" + getInpCl(siteList.d[this.rel]), "value":getInpVal(siteList.d[this.rel])});
    ...
    td[2].grab(inpUrl);
    td[3].empty().grab(new Element("a", {"href":"", "class":"save2", "rel":this.rel, events:{"click":function(e) {
      if (e) {
        e.stop();
      }
...
function genAct(key) {
  var acts = new Element("td", {"class":"text-right"});
  if (siteList.d[key].u != -5) {
    acts.grab(new Element("a", {"href":"", "class":"edit", "rel":key, events:{"click":frigateEditList}}));
    acts.grab(new Element("a", {"href":"", "class":"trbut", "rel":key, events:{"click":frigateDelList}}));
  }
  return acts;
}
function genHost(key, val) {
  var clHost = getClHost(val.h);
  var valh = val.h;
  if (intertld(val.h)) {
    valh = punycode.ToUnicode(val.h);
  }
  return (new Element("td")).grab(new Element("img", {src:"http://" + clHost + "/favicon.ico", width:18, styles:{"vertical-align":"middle", "margin-right":"5px", "display":"inline-block", "padding-bottom":"1px"}, events:{"error":function() {
    this.set("src", "im/folder.png");
  }}})).grab(new Element("a", {"text":valh, "class":"nolink bold", "rel":key, "href":"http://" + clHost, "target":"_blank", styles:{}, events:{}}));
}
function showList() {
  var list = siteList.d;
  ...
  if (list && list.length > 0) {
    list.sort(function(a, b) {
      ...
      bh = getClHost(b.h);
      if (intertld(ah)) {
        ah = punycode.ToUnicode(ah);
      }
      if (intertld(bh)) {
        bh = punycode.ToUnicode(bh);
      }
      if (ah == bh) {
        return 0;
      }
    ...
    Object.each(list, function(val, key) {
      ...
      tr = new Element("tr", {"class":className, "id":"list" + key});
      tr.grab(new Element("td", {"text":i}));
      tr.grab(genHost(key, val));
      if (val.u != -5) {
        tr.grab((new Element("td")).grab(new Element("a", {"text":cutUrl(val.u), "class":"nolink", "rel":key, "href":"", events:{"click":noact, "dblclick":frigateEditList}})));
        tr.grab(genAct(key));
      }
      ...
      tr.grab((new Element("td", {"class":"text-center"})).grab(new Element("a", {"href":"", "class":classonoff, "rel":key, events:{"click":frigateOnOffList}})));
      tableObj.grab(tr);
    });
...
window.addEvent("domready", function() {
  ...
  if (params.id == -1) {
    chrome[runtimeOrExtension].sendMessage({type:"frigate", value:"getfrlist"}, function(resp) {
      var dat = [];
      Object.each(resp, function(val, key) {
        if (val.hasOwnProperty("lid") && val.lid > -1) {
          return;
        }
        var todat = {h:key, u:-5, on:true};
        if (val.ons < 0) {
          todat.on = false;
        }
        dat.push(todat);
      });
      siteList = {n:"friGate list", on:true, d:dat};
