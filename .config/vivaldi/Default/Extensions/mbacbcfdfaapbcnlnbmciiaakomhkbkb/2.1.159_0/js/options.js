...
function showProxy() {
  ...
  if (pr && pr.length > 0) {
    ...
    Array.each(pr, function(val, key) {
      ...
      proxy_list.grab((new Element("div", {html:newval + "</strong>"})).grab(new Element("a", {text:" ", "class":"trbut", rel:val, events:{click:frigateDelProxy}}), "top"));
    });
  } else {
...
function frigateDelList(event) {
  ...
  SM.show({"model":"confirm", "callback":function() {
    chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"offlist", "id":this.rel}}, function(resp) {
      delete lsList[this.rel];
      lsList = lsList.clean();
      ls.set("list", lsList);
      showList();
    }.bind(this));
  }.bind(this), "title":l("messModalConfirmTitle"), "contents":mess});
}
function frigateOnOffList(event) {
  ...
  if (typeof lsList[this.rel].on == "undefined" || !lsList[this.rel].on) {
    chrome[runtimeOrExtension].sendMessage({type:"frigatelist", value:{"act":"onlist", "id":this.rel}}, function(resp) {
      this.set("class", "on");
...
function frigateEditList(event) {
  ...
  var inp = new Element("input", {"id":"listname" + this.rel, "class":"inpedit", "value":lsList[this.rel].n});
  td.grab(inp).grab(new Element("a", {"href":"#", "class":"save", "rel":this.rel, events:{"click":function(e) {
    if (e) {
      e.stop();
    }
...
function showList() {
  ...
  if (lsListShow && !emptyObject(lsListShow)) {
    ...
    Object.each(lsListShow, function(val, key) {
      ...
      col = Element("td", {text:val.d.length});
      tr = new Element("tr", {"class":className});
      listname = new Element("td", {"id":"list" + (key - 1), "html":'<h4><span class="folder">&nbsp;</span><a href="' + chrome.extension.getURL("list.html") + "?id=" + (key - 1) + '">' + val.n + "</a></h4>"});
      tr = tr.grab(listname);
      tr = tr.grab(col);
      if (key == 0) {
        ...
        acts.grab(new Element("a", {"href":"#", "class":"edit", "rel":key - 1, events:{"click":frigateEditList}}));
        acts.grab(new Element("a", {"href":"#", "class":"trbut", "rel":key - 1, events:{"click":frigateDelList}}));
        tr.grab(acts);
        tr.grab((new Element("td", {"class":"text-center"})).grab(new Element("a", {"href":"#", "class":classonoff, "rel":key - 1, events:{"click":frigateOnOffList}})));
      }
      list_table.grab(tr);
    });
...
window.addEvent("domready", function() {
  ...
  getTld();
  var tld_list = $("tld_list").empty();
  for (var key in tld) {
    if (tld.hasOwnProperty(key)) {
      var inputtld = new Element("input", {"type":"checkbox", "id":key, "checked":tld[key] ? "checked" : "", events:{mouseup:function() {
        tld[this.id] = !this.checked;
        setTld(tld);
        chrome[runtimeOrExtension].sendMessage({type:"frigate", value:"gettld"});
      }}});
      tld_list.grab(inputtld).appendText("." + key + " ");
    }
  }
  ...
  chrome[runtimeOrExtension].sendMessage({type:"frigate", value:"getfrlist"}, function(resp) {
    ...
    frList = {n:"friGate", on:true, d:dat};
    showList();
  });
