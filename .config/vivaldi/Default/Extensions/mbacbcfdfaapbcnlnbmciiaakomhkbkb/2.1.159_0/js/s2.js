...
if (typeof frigate_s2 !== "function") {
  ...
  function frigate_s2() {
    ...
    var frigate_s2_close = function(e) {
      ...
      if (!$("frigate_wr2")) {
        var head = (new Element("div", {"id":"frigate_wr2"})).grab((new Element("div", {"class":"frigate_h", "text":""})).grab(new Element("a", {"class":"frigate_link2", "html":"&nbsp;", "data-tooltip":"open", events:{"click":frigate_s2_open}})));
        document.html.grab(head);
        head.setStyle("visibility", "visible");
      }
      chrome[runtimeOrExtensionFrigate].sendMessage({"type":"from_s2", "tabHost":tabHost, "value":true});
    };
    var frigate_s2_open = function(e) {
      ...
      chrome[runtimeOrExtensionFrigate].sendMessage({"type":"from_s2", "tabHost":tabHost, "value":false});
    };
    ...
    var showMainWin = function() {
      ...
      mainWinIsShow = true;
      document.html.grab((new Element("div", {"id":"frigate_wr", styles:{display:"none"}, "class":"frigate_vi"})).grab((new Element("div", {"id":"frigate_topbl"})).grab(new Element("div", {"class":"frigate_h", "html":"friGate &nbsp;&nbsp;"})).grab((new Element("div", {"id":"frigate_body"})).grab(new Element("div", {"id":"frigate_list", "class":"frigate_highlight"})).grab(new Element("a", {"id":"frigate_on", "class":"frigate_links frigate_off", "html":"&nbsp;"})).grab(new Element("div", {"id":"frigate_dop", 
      "class":"frigate_small"}))).grab((new Element("div", {"id":"frigate_f"})).grab(new Element("a", {"class":"frigate_link", "html":"&nbsp;", "data-tooltip":"close", events:{"click":frigate_s2_close}})))));
    };
    var isreptext = false;
        ...
        if (request.type == "s2" && request.value) {
          ...
          if (!isGetMessageStart) {
            ...
            window.addEvent("domready", function() {
              var ScrollSpy = new Class({Implements:[Options, Events], options:{min:0, mode:"vertical", max:0, container:window, onEnter:"", onLeave:$empty, onTick:$empty}, initialize:function(a) {
                this.setOptions(a);
              ...
              var topbar = $("frigate_wr").set("tween", {duration:200}), topDistance = 30, fadeTo = .5;
              var topbarME = function() {
                topbar.tween("opacity", 1);
              }, topbarML = function() {
                topbar.tween("opacity", fadeTo);
              };
              var events = {mouseenter:topbarME, mouseleave:topbarML};
              var ss = new ScrollSpy({min:topDistance, max:window.getScrollSize().y + 1E3, onLeave:function() {
                topbarME();
            ...
            if (request.u) {
              frigate_body.grab(new Element("div", {"html":request.u}));
            }
            if (request.n) {
              frigate_body.grab(new Element("div", {"html":request.n}));
            }
          }
          ...
          if (request.value.isonepage == 3 || request.value.isonepage == 4 || request.value.isonepage == 1) {
            if (request.pr[1]) {
              frigate_dop.grab(new Element("a", {"class":"frigatel_" + request.pr[1] + " frigate_links", "html":"&nbsp;", "data-tooltip":request.pr[0], events:{"click":chProxy}}));
            }
          }
          if (!request.hide) {
            $("frigate_wr").setStyle("visibility", "visible");
          ...
          if (request.type == "s2r" && request.rep.length > 0) {
            if (isreptext) {
              return;
            }
            isreptext = true;
